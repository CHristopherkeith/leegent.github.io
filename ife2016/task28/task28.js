...
var BUS = {
    ...
    adapter : {
        ...
        encodeCraftInfo:function(jsonInfo){
            ...
            var biInfo = "";
            // 前四位标示飞船编号
            biInfo += parseInt(jsonCommand.id, 10).toString(2);
            while (biInfo.length < 4) {
                biInfo = "0" + biInfo;
            }
            // 4~8位表示飞船状态
            if(biInfo.state === "MOVE") {
                biInfo +="0001";
            }
            else if(biInfo.state === "STAY"){
                biInfo +="0010";
            }
            else{
                biInfo +="1100";
...
var planet = {
    ...
    spacecraftFactory:function(o) {
        ...
        var newId = -1;
        //选择一个空闲的编号
        if (BUS.observers.length == 0) newId = 1;
        else {
            for (var i = 0; i < BUS.observers.length; i++) {
                if (BUS.observers[i].getId() > i + 1) {
                    newId = i + 1;
                    break;
                }
            }
            if (newId === -1) newId = BUS.observers.length + 1;
        }
        // 设置用于构造新船的属性
        obj.id = newId;
     ...
     */
     ...
     DC:{
        ...
        display:function(binaryInfo){
            ...
            // 取出飞船信息所在行
            var line = this.craftsData[jsonInfo.id];
            // 取不到说明是新飞船，添加
            if(!line){
        ...
        // 添加新飞船
        addCraft:function(jsonInfo){
            this.craftsData[jsonInfo.id] = ;
        }
     }
};
 ...
 * 飞船暴露给外界的接口有2个：getId()和onReceiveMessage()
 */
function Spacecraft (obj) {
     ...
     */
    var _signalSender = setInterval(function(){
        var selfInfo = {
            id:that._id,
            state:that._state,
            energy:that._energy
        };
        BUS.onReceive(BUS.craftInfoEncoder(selfInfo));
    },1000);
    /*
     * 飞船对外提供两个接口
     * */
    ...
    // 2.接收并执行命令
    this.onReceiveMessage = function (bicmd) {
        // 解码
...
$(document).ready(function () {
   ...
   .init();
    // 将行星上的信号接收器添加到BUS
    BUS.addObserver(planet.signalReceiver);
});